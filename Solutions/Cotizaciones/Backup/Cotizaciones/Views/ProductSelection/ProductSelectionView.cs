    ...
    public partial class ProductSelectionView : BaseView
    {
        ...
        private void InitializeQuotationData(DataTable qdt)
        {
            foreach (DataRow row in qdt.Rows)
            {
                ...
                newRow["TSC42"] = row["TSC42"];
                newRow["TSC52"] = row["TSC52"];
                newRow["ERW"] = row["ERW"];
                newRow["LSAW"] = row["LSAW"];
                newRow["SSAW"] = row["SSAW"];
            }
        }
        ...
        private bool IsValidData()
        {
            ...
            int sID;
            ArrayList pipeSpecificationIds = new ArrayList();
            ...
            if (this.ultraDataSourceProducts.Rows.Count > 0)
            {
                foreach (UltraDataRow row in this.ultraDataSourceProducts.Rows)
                {
                    if (row["Diámetro Nominal"] != DBNull.Value && row["Espesor"] != DBNull.Value && row["Espesor"].ToString() != "")
                    {
                        ...
                        sID = FindSpecification(dtAllSpecs, FindDiameter(dtAllSpecs, row["Diámetro Nominal"].ToString()), row["Espesor"].ToString());
                        if (!pipeSpecificationIds.Contains(sID))
                        {
                            pipeSpecificationIds.Add(sID);
                        }
                        else
                        {
                            if (!dupRows.Contains(errorValue))
                            {
                                dupRows.Add(errorValue);
                            }
                        }
                        //Check for non used rows
                        if (!Convert.ToBoolean(row["TSC42"]) &&
                            !Convert.ToBoolean(row["TSC52"]) &&
                            !Convert.ToBoolean(row["ERW"]) &&
                            !Convert.ToBoolean(row["LSAW"]) &&
                            !Convert.ToBoolean(row["SSAW"])
                            )
                        {
            ...
            if (dupRows.Count > 0)
            {
                errorMessage += "Elementos duplicados: " + Environment.NewLine;
                foreach (string element in dupRows)
                {
        ...
        private void UpdateQuotation()
        {
            if (!IsReadOnly)
            {
                ...
                foreach (UltraDataRow row in this.ultraDataSourceProducts.Rows)
                {
                    ...
                    inputRow["TSC42"] = Convert.ToBoolean(row["TSC42"]);
                    inputRow["TSC52"] = Convert.ToBoolean(row["TSC52"]);
                    inputRow["ERW"] = Convert.ToBoolean(row["ERW"]);
                    inputRow["LSAW"] = Convert.ToBoolean(row["LSAW"]);
                    inputRow["SSAW"] = Convert.ToBoolean(row["SSAW"]);
                    inputData.Rows.Add(inputRow);
                }
                ProductSelectionManager.UpdateQuotationDetails(this.QuotationID, this.User, inputData, OriginalProductInfo);
            }
        }
        ...
        private void ugProducts_AfterCellUpdate(object sender, Infragistics.Win.UltraWinGrid.CellEventArgs e)
        {
            switch (e.Cell.Column.Key)
            {
                ...
                case "Peso":
                    break;
                default:
                    //MessageBox.Show(e.Cell.Column.Key);
                    break;
            }
        }

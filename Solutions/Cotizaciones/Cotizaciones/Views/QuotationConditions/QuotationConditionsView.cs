...
namespace Cotizaciones.Views
{
    public partial class QuotationConditionsView : BaseView
    {
        ...
        private bool IsValidData()
        {
            ...
            foreach (DataRow row in QuotationData.Tables["Table1"].Rows)
            {
                if (row["Weight"].ToString().Length > 20)
                {
                    errorMessage += "Texto muy largo en columna de Peso en " + row.GetParentRow(QuotationData.Relations[0])["ShortName"].ToString() + ", Diámetro " + row["ShortName"].ToString() + ", Espesor " + row["Description"].ToString() + Environment.NewLine;
                    result = false;
                }
            }
            foreach (DataRow row in QuotationData.Tables["Table"].Rows)
            {
                if (row["Description"].ToString().Trim() == String.Empty)
                {
                    errorMessage += "Introduzca la descripción de la sección " + row["ShortName"].ToString() + Environment.NewLine;
                    result = false;
                }
            }
            if (!result)
            ...
            foreach (UltraGridColumn col in this.ugQuotationConditions.DisplayLayout.Bands[1].Columns)
            {
                switch (col.Key)
                {
                    ...
                    case "Description":
                        ...
                        break;
                    case "Quantity":
                        col.Header.Caption = "Cantidad requerida";
                        col.CellActivation = Activation.NoEdit;
                        break;
                    case "Price":
                    ...
                    case "TotalPerConcept":                        
                        ...
                        break;
                    case "LinearWeight":
                        col.Header.Caption = "Peso Lineal";
                        col.CellActivation = Activation.NoEdit;
                        col.Format = "#.#0";
                        break;
        ...
        private void ugQuotationConditions_CellChange(object sender, CellEventArgs e)
        {
            if (ugQuotationConditions.ActiveCell.Column.Key == "Price" && e.Cell.Row.Cells["TotalPerConcept"].Text != "Variable")
            {
                try
                {
                    double Price;
                    if (ugQuotationConditions.ActiveCell.Text == "")
                    { Price = 0; }
                    else
                        Price = Convert.ToDouble(ugQuotationConditions.ActiveCell.Text);
                    double Quantity = Convert.ToDouble(e.Cell.Row.Cells["Quantity"].Text);
                    string Total = string.Format("$ {0:###,###.#0}", (Price * Quantity));
                    e.Cell.Row.Cells["TotalPerConcept"].Value = Total;
                }
                catch { }
            }
        }

        private void ugQuotationConditions_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (ugQuotationConditions.ActiveCell.Column.Key == "Price")
            {
                ...
                e.Handled = false;
                      ...
                      if (char.IsControl(e.KeyChar))
                            ...
                            {
                                if (!ugQuotationConditions.ActiveCell.Text.Contains('.'))
                                    e.Handled = false;
                                else
                                    e.Handled = true;
                            }
